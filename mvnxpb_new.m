...
%
% ADAPTATIONS: diagonal is 1, only upper bound provided, lower bound -Inf.
[n,n] = size(r); c = r; bp = b; ep = 1e-10; % singularity tolerance;
d = sqrt(max(diag(c),0)); 
...
for k = 1 : n, im = k; ckk = 0; dem = 1; s = 0; 
  for i = k : n 
    if c(i,i) > eps, cii = sqrt( max( [c(i,i) 0] ) ); 
      ...
      bi = ( bp(i)-s )/cii; de = Phi(bi);
      if de <= dem, ckk = cii; dem = de; bm = bi; im = i; end
    end
  end
  ...
  if ckk > ep*k, c(k,k) = ckk; c(k,k+1:n) = 0; 
    ...
    if abs(dem) > ep, y(k) =  - phi(bm)/dem; 
    else, 
      y(k) = bm; 
    end
  else, c(k:n,k) = 0; y(k) = bp(k)/2;
  end, p = p*dem;
  ...
  if mod(k,2) == 0, u = c(k-1,k-1); v = c(k,k); w = c(k,k-1); 
    ...
    [Ex Ey bv] = bvnmmg( -Inf*[1,1], bb-cb, sg ); 
