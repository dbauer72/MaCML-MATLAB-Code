function [lp,gr,lpi] = cal_prob_like(mod,data,fun,theta)
...
if nargin ==4
    ...
    [lp,gr,lpi] = cal_prob_like_ccml(mod,data,fun);
    return
end;
...
for i=1:n
    ...
    if D>1
        % only consecutive decisions for D decisions.
        for c= 1:(D-1)
            % calc kk1, kk2.
            T = nO(c)+nO(c+1); % number of options avail - numbre of dec.
            kk1 = zeros(T,1);
            kk2 = zeros(T,T);
            ...
            cur = 0;
            for d=[c,c+1] % cycle over indiv. decisions
                if co
                ...
                d_Sig(cur+[1:nO(d)],cur+[1:nO(d)])=squeeze(MSigma(in,in,yd(d)));
        ...
        % calc deriv
        gri(1:nvarb)= gr(1:nvarb)- (grii(1:T)./sd')*dX*db; % der. w.r.t. entries in b.
